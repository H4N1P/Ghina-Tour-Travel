                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    @if ($p->isFinal())
                                        <button type="button"
                                            data-admin-notice
                                            data-admin-notice-type="danger"
                                            data-admin-notice-title="Pesanan Tidak Dapat Diedit"
                                            data-admin-notice-message="Pesanan berstatus {{ ucfirst($p->status) }} sudah final dan tidak dapat diubah."
                                            data-admin-notice-confirm="Mengerti"
                                            aria-label="Edit pesanan tidak tersedia karena status {{ $p->status }} sudah final"
                                            title="Status {{ ucfirst($p->status) }} sudah final"
                                            class="flex min-h-11 min-w-11 cursor-pointer items-center justify-center rounded-lg text-admin-muted opacity-60 transition-colors hover:bg-admin-bg hover:opacity-100">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
