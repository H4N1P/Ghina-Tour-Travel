                ...
                <tbody>
                    @forelse($pesanans as $p)
                        <tr
                            ...
                            <td class="px-4 py-3 text-sm">{{ $p->paket->nama_paket ?? ($p->destinasi_custom ?? 'Custom') }}
                            </td>
                            <td class="px-4 py-3">
                                <span
                                    class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold {{ $p->tipe_pesanan === 'custom' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700' }}">
                                    {{ ucfirst($p->tipe_pesanan ?? 'default') }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-sm font-semibold text-green-700">Rp
                                {{ number_format($p->total_harga, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-sm text-red-500">
                                {{ $p->diskon > 0 ? '- Rp ' . number_format($p->diskon, 0, ',', '.') : '-' }}</td>
                            <td class="px-4 py-3">
                                <span
                                    class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold {{ $p->status === 'selesai' ? 'bg-green-100 text-green-700' : ($p->status === 'batal' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">
                                    {{ $p->status ?? 'Pending' }}
                                </span>
                            </td>
