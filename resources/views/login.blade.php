    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <div class="flex min-h-[50px] items-center gap-3 rounded-full border-[1.5px] {{ $errors->has('email') ? 'border-red-600' : 'border-[#e8dfc8]' }} bg-[#FBF5E6] px-4 focus-within:border-[#B8952A]">
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="Masukkan Email" autocomplete="email" autofocus class="min-w-0 flex-1 bg-transparent text-sm text-[#3D2008] outline-none placeholder:text-[#c4a97a]">
            </div>
            @error('email')
        ...
        <div>
            <div class="mb-2 flex items-center justify-between gap-3">
                <label for="password" class="block text-[11px] font-bold uppercase tracking-[1px] text-[#8a7050]">Password</label>
                <a href="{{ route('password.request') }}" class="inline-flex min-h-11 items-center text-xs font-semibold text-[#B8952A] hover:text-[#8a6e1a]">Forgot Password?</a>
            </div>
            <div class="flex min-h-[50px] items-center gap-3 rounded-full border-[1.5px] {{ $errors->has('password') ? 'border-red-600' : 'border-[#e8dfc8]' }} bg-[#FBF5E6] px-4 focus-within:border-[#B8952A]">
                <svg class="h-[18px] w-[18px] shrink-0 stroke-[#B8952A]" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
                    <path d="M7 11V7a5 5 0 0 1 10 0v4" />
                </svg>
                <input type="password" id="password" name="password" placeholder="Password" autocomplete="current-password" class="min-w-0 flex-1 bg-transparent text-sm text-[#3D2008] outline-none placeholder:text-[#c4a97a]">
...
@section('back_link')
    <a href="{{ url('/') }}" class="mt-5 inline-flex min-h-11 items-center justify-center gap-2 text-sm text-[#8a7050] hover:text-[#3D2008]">
