...
@section('content')
    <!-- HERO / HEADER PAKET -->
    <div class="detail-hero relative h-[260px] w-full overflow-hidden mt-[72px]">
        ...
        <div class="absolute inset-0 flex items-center justify-center flex-col gap-2 px-4 text-center">
            <h1 class="text-[38px] font-bold {{ $paket->fotos && $paket->fotos->count() > 0 ? 'text-white' : 't' }}">
                {{ $paket->nama_paket }}
            </h1>
            <p class="{{ $paket->fotos && $paket->fotos->count() > 0 ? 'text-gray-200' : 'tm' }} text-base">Paket Wisata</p>
        </div>
    </div>
    ...
    <main class="mx-auto max-w-[1280px] px-6 lg:px-14 py-10">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach ($paket->tempats as $tempat)
                        <div class="tempat-card">
                            ...
                            @endif
                            <div class="tempat-card__label">{{ $tempat->nama_tempat }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
                        ...
                        @foreach ($paket->rundowns as $index => $rundown)
                            <div class="rundown-card">
                                <div class="flex items-start gap-4">
                                    <span class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-black font-bold text-sm" style="background:var(--gold);">{{ $index + 1 }}</span>
                                    <div class="flex-1">
                                        <p class="font-semibold text-[15px]" style="color:var(--gold-dark);">{{ $rundown->waktu }}</p>
        ...
        <div class="flex gap-8 flex-col lg:flex-row">
            ...
            <div class="fas-box flex-1 rounded-2xl p-7">
                ...
                @php
                    $transportasis = $paket->fasilitas ? $paket->fasilitas->where('tipe_fasilitas', 'transportasi') : collect();
                    $akomodasis = $paket->fasilitas ? $paket->fasilitas->where('tipe_fasilitas', 'akomodasi') : collect();
            ...
            <div class="w-full lg:w-[300px] flex-shrink-0 space-y-4">
                <div class="harga-box rounded-2xl p-6 space-y-3">
                    ...
                    <p class="text-[26px] font-bold t">
                        Rp {{ number_format($paket->harga_paket, 0, ',', '.') }}<span class="text-base font-normal tm">/pax</span>
                    </p>
                    ...
                    <div class="space-y-2 text-sm tm">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 flex-shrink-0" style="color:var(--gold);" fill="currentColor" viewBox="0 0 24 24">
